...
clf= joblib.load(MODELS_PATH + "/Profane_language_detection/logreg_clf")
tf_idf_vector = joblib.load(MODELS_PATH + "/Profane_language_detection/tfidf_vectorizer")
...
def detector_from_csvfile(csv_filename):
    ...
    data["Recommended_Action"] = np.where(data['Result'] == "Non Profane Language", "Informative, Like/Retweet", "Can be Deleted")
    #data["Action"] = data['Probability'].apply(lambda x :action(x))
    #data["Action"] = np.where(data['Result'] == "Neither",data["Action"],"Can be Removed")
...
def detector_from_string(string):
    ...
    if arr[0] == 0:
        ...
        d['Result'] = "It contains profane language"
    else:
        d['Result'] = "It's neither a hate speech nor containing profane language"
